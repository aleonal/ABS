...
class RunnerWidget(QWidget):
    ...
    def display_script(self):
        try:
            ...
            validator_check = Path(self.script_name.replace(".py", "Validator.json"))
            if validator_check.exists():
                ...
                self.script_name = None
                self.invalid_script_message()
        except Exception as e:
            pass

    # Notifies user if they try to load an invalid script
    def invalid_script_message(self):
        messageBox = QMessageBox()
        messageBox.setWindowTitle("Error")
        messageBox.setText("Invalid script selected. Please select a script generated by the Builder.")
        messageBox.exec()
